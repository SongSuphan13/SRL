            ...
            <div class="card">
                <div id="wf_space" class="card-body">
                    <form name="form_save" id="form_save" method="post" action="#">
                        <input type="hidden" name="srl_id" value="<?php echo isset($dataObj['prefix_id'])?$dataObj['prefix_id']:''; ?>" />
                        <div class="form-group row">
                            ...
                            <!-- <label for="bank_code" class="col-md-2 col-form-label hm-right">รหัสคำนำหน้า <span style="color: #f46a6a">*</span></label>
                            <div class="col-md-3">
                        ...
                        <div class="form-group row">
                            <label for="prefix_name_th" class="col-md-2 col-form-label hm-right">คำนำหน้า (TH) <span style="color: #f46a6a">*</span></label>
                            <div class="col-md-3">
                                <input type="text" id="prefix_name_th" name="prefix_name_th" class="form-control" value="<?php echo isset($dataObj['prefix_name_th'])?$dataObj['prefix_name_th']:''; ?>" required/>
                             </div>
                            <label for="prefix_name_en" class="col-md-2 col-form-label hm-right">คำนำหน้า (EN)</label>
                            <div class="col-md-3">
                                <input type="text" id="prefix_name_en" name="prefix_name_en" class="form-control" value="<?php echo isset($dataObj['prefix_name_en'])?$dataObj['prefix_name_en']:''; ?>" />
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="prefix_nameshort_th" class="col-md-2 col-form-label hm-right">คำนำหน้าย่อ (TH)</label>
                            <div class="col-md-3">
                                <input type="text" id="prefix_nameshort_th" name="prefix_nameshort_th" class="form-control" value="<?php echo isset($dataObj['prefix_nameshort_th'])?$dataObj['prefix_nameshort_th']:''; ?>" />
                            </div>
                            <label for="prefix_nameshort_en" class="col-md-2 col-form-label hm-right">คำนำหน้าย่อ (EN)</label>
                            <div class="col-md-3">
                                <input type="text" id="prefix_nameshort_en" name="prefix_nameshort_en" class="form-control" value="<?php echo isset($dataObj['prefix_nameshort_en'])?$dataObj['prefix_nameshort_en']:''; ?>" />
                            </div>
                        </div>
                        <div class="form-group row">
...
<?=$this->section('combottom_js_user')?>
    <script>
        function get_save(){
            ...
            if($('input[id=prefix_name_th]:required').val() == ''){
                $('input[id=prefix_name_th]:required').addClass("is-invalid");
                $('input[id=prefix_name_th]:required').after(brfore+'กรุณาระบุคำนำหน้า (TH)'+after)
            }
            ...
            swal({
                ...
                $.ajax({
                    type: "POST",
                    url: baseUrl+ "/setting/prefix/save",
                    processData: false,
                    contentType: false,
                    dataType: 'json',
                    ...
                    success: function (response) {
                        if(response.statusCode == 150){
                            showSuccess('บันทึกข้อมูลเรียบร้อย','<?php echo base_url('setting/prefix') ?>');
                        }else{
