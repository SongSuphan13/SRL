                ...
                <div id="wf_space" class="card-body">
                    <form name="form_save" id="form_save" method="post" action="#">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-3">
                                <select id="province_code" name="province_code" class="form-control select2" onchange="get_amphure('province_id','amphure_id','tambon_id','zipcode','')" required>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-3">
                                <input type="text" id="amphure_name_th" name="amphure_name_th" class="form-control" value="<?php echo isset($objData['amphure_name_th'])?$objData['amphure_name_th']:''; ?>" required/>
                             </div>
                            <label for="amphure_name_en" class="col-md-2 col-form-label hm-right">อำเภอ (EN)</label>
                            <div class="col-md-3">
                                <input type="text" id="amphure_name_en" name="amphure_name_en" class="form-control" value="<?php echo isset($objData['amphure_name_en'])?$objData['amphure_name_en']:''; ?>" />
                            </div>
                        </div>
                    </form>
                </div>
...
<?=$this->section('combottom_js_user')?>
    <script>
        function get_save(){
            ...
            if($('input[id=amphure_name_th]:required').val() == ''){
                $('input[id=amphure_name_th]:required').addClass("is-invalid");
                $('input[id=amphure_name_th]:required').after(brfore+'กรุณาระบุอำเภอ (TH)'+after)
                required++;
            }
            if(required > 0){
                return false;
            ...
            swal({
                ...
                $.ajax({
                    type: "POST",
                    url: baseUrl+ "/setting/amphure/save",
                    processData: false,
                    contentType: false,
                    dataType: 'json',
                    ...
                    success: function (response) {
                        if(response.statusCode == 150){
                            showSuccess('บันทึกข้อมูลเรียบร้อย','<?php echo base_url('setting/amphure/') ?>');
                        }else{
