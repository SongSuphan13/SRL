...
<?php 
     function get_setting_menu($m_id,$group_id){
        ...
        if(count($sql_menu_group)> 0){
            ...
            $r_group = $db->query("select * from aut_group_menu where 1=1 and menu_id = ".$m_id." and group_id = '".$group_id."' and user_type = 'G' ")->getRowArray();
    ...
    <script>
        function get_save(){
            swal({
                ...
                $.ajax({
                    type: "POST",
                    url: baseUrl+ "/user_account/aut_group/save_menu", 
                    processData: false,
                    contentType: false,
                    data: fdata,
