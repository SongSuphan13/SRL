                ...
                <div id="wf_space" class="card-body">
                    <form name="form_save" id="form_save" method="post" action="#">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-3">
                                <input type="text" id="chemi_seq" name="chemi_seq" class="form-control text-right" value="<?php echo isset($objData['chemi_seq'])?$objData['chemi_seq']:''; ?>" onkeypress="return isNumber(event)" required/>
                            </div>
                            <label for="chemi_code" class="col-md-2 col-form-label hm-right">รหัสกลุ่มสารเคมี <span style="color: #f46a6a">*</span></label>
                            <div class="col-md-3">
                                <input type="text" id="chemi_code" name="chemi_code" class="form-control" value="<?php echo isset($objData['chemi_code'])?$objData['chemi_code']:''; ?>" required/>
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="chemi_name_th" class="col-md-2 col-form-label hm-right">กลุ่มสารเคมี (TH) <span style="color: #f46a6a">*</span></label>
                            <div class="col-md-3">
                                <input type="text" id="chemi_name_th" name="chemi_name_th" class="form-control" value="<?php echo isset($objData['chemi_name_th'])?$objData['chemi_name_th']:''; ?>" required/>
                             </div>
                            <label for="chemi_name_en" class="col-md-2 col-form-label hm-right">กลุ่มสารเคมี (EN)</label>
                            <div class="col-md-3">
                                <input type="text" id="chemi_name_en" name="chemi_name_en" class="form-control" value="<?php echo isset($objData['chemi_name_en'])?$objData['chemi_name_en']:''; ?>" />
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="chemi_nameshort_th" class="col-md-2 col-form-label hm-right">กลุ่มสารเคมีย่อ (TH)</label>
                            <div class="col-md-3">
                                <input type="text" id="chemi_nameshort_th" name="chemi_nameshort_th" class="form-control" value="<?php echo isset($objData['chemi_nameshort_th'])?$objData['chemi_nameshort_th']:''; ?>" required/>
                             </div>
                            <label for="chemi_nameshort_en" class="col-md-2 col-form-label hm-right">กลุ่มสารเคมีย่อ (EN)</label>
                            <div class="col-md-3">
                                <input type="text" id="chemi_nameshort_en" name="chemi_nameshort_en" class="form-control" value="<?php echo isset($objData['chemi_nameshort_en'])?$objData['chemi_nameshort_en']:''; ?>" />
                            </div>
                        </div>
                        <div class="form-group row">
...
<?=$this->section('combottom_js_user')?>
    <script>
        function get_save(){
            ...
            var after = '</div>';
            var required = 0;
            if($('input[id=chemi_seq]:required').val() == ''){
                $('input[id=chemi_seq]:required').addClass("is-invalid");
                $('input[id=chemi_seq]:required').after(brfore+'กรุณาระบุลำดับ'+after)
                required++;
            }
            if($('input[id=chemi_code]:required').val() == ''){
                $('input[id=chemi_code]:required').addClass("is-invalid");
                $('input[id=chemi_code]:required').after(brfore+'กรุณาระบุรหัสกลุ่มสารเคมี'+after)
                required++;
            }
            if($('input[id=chemi_name_th]:required').val() == ''){
                $('input[id=chemi_name_th]:required').addClass("is-invalid");
                $('input[id=chemi_name_th]:required').after(brfore+'กรุณาระบุกลุ่มสารเคมี (TH)'+after)
                required++;
            }
            ...
            swal({
                ...
                $.ajax({
                    type: "POST",
                    url: baseUrl+ "/product/chemical_group/save",
                    processData: false,
                    contentType: false,
                    dataType: 'json',
                    ...
                    success: function (response) {
                        if(response.statusCode == 150){
                            showSuccess('บันทึกข้อมูลเรียบร้อย','<?php echo base_url('product/chemical_group/') ?>');
                        }else{
